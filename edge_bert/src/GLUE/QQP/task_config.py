...
def training_config(regimen: str | None = None) -> dict[str, Any]:
    ...
    if target not in TRAINING_CONFIGS:
        raise ValueError(f"Unsupported QQP regimen: {target}")
...
def load_greedy_config(required: bool = True) -> dict[str, int]:
    ...
    if isinstance(config, dict):
        return config
    if required:
        raise FileNotFoundError(f"Missing QQP greedy config: {greedy_config_path()}")
    return {}
...
def load_sa_best_config(required: bool = True) -> dict[str, int]:
    ...
    if required:
        raise FileNotFoundError(f"Missing QQP SA config: {sa_best_config_path()}")
    return {}
