...
from GLUE.QQP.task_config import GREEDY_SETTINGS, greedy_config_path, load_sensitivity_results, save_json


def main() -> None:
    sensitivity_results = load_sensitivity_results()
    if not sensitivity_results:
        raise FileNotFoundError("Run QQP analyze_layer_sensitivity.py first to generate sensitivity results.")
    ...
    if GREEDY_SETTINGS["allocation_strategy"] != "tertiles":
        raise ValueError(f"Unsupported QQP greedy allocation strategy: {GREEDY_SETTINGS['allocation_strategy']}")
    ...
    print(f"QQP greedy bit allocation saved to {greedy_config_path()}")
