...
from GLUE.QQP.common import load_qqp_dataset
...
def _build_state(*, verbose: bool = False) -> dict[str, Any]:
    if verbose:
        print("Loading QQP cached search reference metrics...")
    ...
    if verbose:
        ...
        print(f"Loading QQP search dataset ({sample_label})...")
    ...
    if verbose:
        print("Loading QQP baseline checkpoint for search...")
...
def get_state(*, verbose: bool = False) -> dict[str, Any]:
    ...
    if not _STATE:
        ...
        print(
            "Reusing cached QQP search context: "
