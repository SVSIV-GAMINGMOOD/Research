...
from GLUE.QQP.common import ensure_cuda_available, evaluate_pytorch_model, load_classifier_checkpoint, load_qqp_dataset
from GLUE.QQP.task_config import EVALUATION_SETTINGS, baseline_checkpoint_path, save_reference_model_results


def evaluate_checkpoint(*, use_half: bool) -> dict[str, object]:
    if use_half:
        ensure_cuda_available("QQP FP16 checkpoint evaluation")
        device = "cuda"
    else:
        device = "cuda" if torch.cuda.is_available() else "cpu"

    dataset = load_qqp_dataset(format_type="torch", max_samples=EVALUATION_SETTINGS["max_samples"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate QQP checkpoint reference models.")
    ...
    results = {
        "strategy": {
            ...
            "notes": "QQP uses FP16 as the primary baseline and FP32 as the research reference.",
        },
        "models": {},
    }
