...
def ensure_cuda_available(context: str) -> None:
    if not torch.cuda.is_available():
        raise RuntimeError(f"{context} requires CUDA for QQP FP16 support, but CUDA is not available.")
...
def export_checkpoint_to_onnx(
    ...
    *,
    ...
    if use_half:
        ensure_cuda_available("QQP FP16 ONNX export")
        export_device = "cuda"
...
def compute_fp16_baseline_onnx_metrics(*, overwrite: bool = False) -> dict[str, Any]:
    ...
    metrics = evaluate_onnx_model(
        baseline_fp16_onnx_path(),
        load_qqp_dataset(format_type="numpy", max_samples=EVALUATION_SETTINGS["max_samples"]),
