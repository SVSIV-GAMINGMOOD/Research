...
from GLUE.MRPC.task_config import FIGURES_DIR, RESULTS_DIR, get_locked_layers, load_sa_best_config
...
def get(result_map: dict, method: str, key: str, default=None):
...
def save_compression_figure(size_results: dict) -> bool:
    fp32_size = get(size_results, "FP32 Research Reference", "theoretical_mb")
    methods = [name for name in MODEL_ORDER if get(size_results, name, "theoretical_mb") is not None]
    if fp32_size is None or not methods:
        return False

    sizes_theory = [get(size_results, method, "theoretical_mb") for method in methods]
    ...
    fig.suptitle(f"{TASK_LABEL} Model Compression Overview", fontweight="bold")
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    plt.savefig(FIGURES_DIR / "fig1_compression_bars.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    return True
...
def main() -> None:
    ...
    size_results = load_json_if_exists(RESULTS_DIR / "size_comparison.json")

    generated: list[str] = []
    if save_compression_figure(size_results):
        generated.append("fig1_compression_bars.png")
