...
from GLUE.MRPC.task_config import FIGURES_DIR, RESULTS_DIR
...
def compute_label_offsets(points: list[tuple[str, float, float]]) -> list[tuple[float, float]]:
    if not points:
        return []

    x_values = [item[1] for item in points]
    y_values = [item[2] for item in points]
    x_span = max(x_values) - min(x_values) if len(x_values) > 1 else 0.0
    y_span = max(y_values) - min(y_values) if len(y_values) > 1 else 0.0
    x_threshold = max(2.0, x_span * 0.08)
    y_threshold = max(0.002, y_span * 0.12 if y_span > 0 else 0.002)
    pattern = [(8, 8), (8, -14), (-52, 8), (-52, -14), (10, 20), (-56, 20)]

    offsets: list[tuple[float, float] | None] = [None] * len(points)
    ...
    while remaining:
        ...
        while changed:
            ...
            for index in list(remaining):
                if any(
                    abs(x_values[index] - x_values[member]) <= x_threshold
                    and abs(y_values[index] - y_values[member]) <= y_threshold
                    for member in cluster
                ):
                    cluster.append(index)
                    remaining.remove(index)
                    changed = True
        cluster.sort(key=lambda idx: (x_values[idx], y_values[idx]))
        for cluster_index, point_index in enumerate(cluster):
            offsets[point_index] = pattern[cluster_index % len(pattern)]
...
def main() -> None:
    ...
    offsets = compute_label_offsets(points)

    for index, (method, size_mb, accuracy) in enumerate(points):
    ...
    ax.set_title("MRPC Accuracy vs Theoretical Model Size", fontweight="bold", color="#1F2933")
