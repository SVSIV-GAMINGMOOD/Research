...
from GLUE.MRPC.optimization.quantization_energy_function import (
    BIT_OPTIONS,
...
from GLUE.MRPC.task_config import FIGURES_DIR, SEARCH_SETTINGS, QUANTIZATION_SETTINGS, RESULTS_DIR, sa_best_config_path, sa_search_results_path, save_sa_search_results
...
def run_simulated_annealing(
    ...
    print(
        f"[MRPC][{threshold_spec['label']}][seed={seed}] "
...
def plot_pareto_frontiers(threshold_results: list[dict], baseline_accuracy: float) -> None:
    ...
    plt.title("MRPC Constrained Pareto Frontiers Across Accuracy Thresholds")
...
def main() -> None:
    print("Preparing MRPC SA search context...")
    ...
    print(
        f"MRPC SA search starting: {len(threshold_specs)} thresholds, {NUM_RUNS} runs each, "
    ...
    print(f"MRPC SA search results saved to {sa_search_results_path()}")
    print(f"MRPC SA best config saved to {sa_best_config_path()}")
